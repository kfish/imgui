...
// These have been moved from imgui.cpp and converted to constexpr
//-----------------------------------------------------------------------------

static constexpr ImVec4 ImColorConvertU32ToFloat4(ImU32 in)
{
    float s = 1.0f / 255.0f;
    return ImVec4(
...
static constexpr ImU32 ImColorConvertFloat4ToU32(const ImVec4& in)
{
    ImU32 out;
...
static constexpr void ImColorConvertRGBtoHSV(float r, float g, float b, float& out_h, float& out_s, float& out_v)
{
    float K = 0.f;
    if (g < b)
    {
...
static constexpr void ImColorConvertHSVtoRGB(float h, float s, float v, float& out_r, float& out_g, float& out_b)
{
    if (s == 0.0f)
    {
        // gray
    ...
    h = h - (int)h; // Equivalent to fmodf(h, 1.0f);
    h /= (60.0f / 360.0f);
    int   i = (int)h;
    float f = h - (float)i;
    float p = v * (1.0f - s);
